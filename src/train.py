...
from models import SSSNET, Balance_Theory
...
class SSSNET_Trainer(object):
    ...
    def SSSNET(self, feat_choice):
        ...
        for split in range(self.splits):
            if not self.args.balance_theory:
                ...
                graphmodel = Balance_Theory(nfeat=self.features.shape[1],
                                    hidden=self.args.hidden,
                                    nclass=self.num_clusters,
                                    dropout=self.args.dropout,
                                    hop=self.args.hop,
            ...
            for epoch in range(args.epochs):
                ...
                model.train()
                logits, output, pred_label, prob = model(self.A_p, self.A_n, self.features, self.A_pt, self.A_nt) 
                if epoch == 0:
                ...
                if not self.args.no_validation:
                    ...
                    if save_perform <= best_val_loss:
                        ...
                        torch.save(model.state_dict(), self.log_path +
                                '/SSSNET_'+feat_choice+'_model'+str(split)+'.t7')
                    else:
                        early_stopping += 1
                ...
                if early_stopping > args.early_stopping or epoch == (args.epochs-1):
                    torch.save(model.state_dict(), self.log_path +
                               '/SSSNET_'+feat_choice+'_model_latest'+str(split)+'.t7')
                    break

            status = 'w'
            if os.path.isfile(self.log_path + '/SSSNET_'+feat_choice+'_log'+str(split)+'.csv'):
                status = 'a'
            with open(self.log_path + '/SSSNET_'+feat_choice+'_log'+str(split)+'.csv', status) as file:
            ...
            if not self.args.no_validation:
                model.load_state_dict(torch.load(
                    self.log_path + '/SSSNET_'+feat_choice+'_model'+str(split)+'.t7'))
                model.eval()
                _,_, pred_label, prob = model(self.A_p, self.A_n, self.features, self.A_pt, self.A_nt) 
                if self.args.SavePred:
                    np.save(self.log_path + '/SSSNET_'+feat_choice +
            ...
            model.load_state_dict(torch.load(
                self.log_path + '/SSSNET_'+feat_choice+'_model_latest'+str(split)+'.t7'))
            model.eval()
            _, _, pred_label, prob = model(self.A_p, self.A_n, self.features, self.A_pt, self.A_nt)
            if self.args.SavePred:
                np.save(self.log_path + '/SSSNET_'+feat_choice +
...
if args.balance_theory:
    save_name += 'balance_theory'
